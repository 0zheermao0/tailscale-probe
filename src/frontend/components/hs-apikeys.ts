...
function render(container: HTMLElement, keys: HeadscaleApiKey[]): void {
  ...
  document.getElementById('hs-create-ak-btn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const tableWrap = container.querySelector<HTMLElement>('table');
      if (tableWrap) tableWrap.outerHTML = buildTable(freshKeys);
      else container.insertAdjacentHTML('beforeend', buildTable(freshKeys));
...
function buildTable(keys: HeadscaleApiKey[]): string {
  ...
  return `<table class="hs-table">
    <thead><tr><th>Prefix</th><th>Created</th><th>Last used</th><th>Expiry</th><th>Actions</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>`;
}
...
function attachActions(container: HTMLElement): void {
  container.querySelectorAll<HTMLElement>('.hs-expire-ak-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      if (!confirm('Expire this API key? It will stop working immediately.')) return;
